  ...
  export class GenericForm implements OnInit, OnChanges {
    // generic-form.ts
    @Input() uniqueCheck?: (fieldName: string, value: any, formValue: any) => Observable<{ exists: boolean; message?: string }>;
    ...
    private buildForm(): void {
      ...
      this.config.forEach(f => {
        ...
        const { sync, async } = this.dynamicValidator.getValidators(f.validations, this.uniqueCheck, f.name);

        if (f.required && !sync.includes(Validators.required)) {
          sync.push(Validators.required);
        }

        group[f.name] = this.fb.control(defaultValue, sync.length ? sync : null, async.length ? async : null);
      });
