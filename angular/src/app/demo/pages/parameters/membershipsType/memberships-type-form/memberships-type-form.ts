...
export class MembershipsTypeForm implements OnInit {
  formConfig: FieldConfig[] = [
    ...
    },
    {
      ...
      validations: [
        ...
        { name: ValidatorNames.Max,      validator: ValidatorNames.Max,      value: 365, message: 'La duración no puede exceder 365 días.' }
      ]
    },
    {
      name: 'priceBase',
  ...
  isEdit = false;
  initialData: Partial<MembershipsType> = {};
  ...
  ngOnInit(): void {
    ...
    if (id) {
      this.isEdit = true;
      this.service.getById<{ success: boolean; data: MembershipsType }>('MemberShipType', id)
        .subscribe(response => {
          if (response.success) {
  ...
  // Validación asíncrona de unicidad (para usarla en <generic-form>)
  uniqueCheck = (fieldName: string, value: unknown, formValue: unknown) =>
    this.uniqueService.checkUnique('MemberShipType', fieldName, value, formValue);
    ...
    if (this.isEdit) {
      this.service.put('MemberShipType', data).subscribe({
        next: () => {
          ...
          this.router.navigate(['/memberShipType-index']);
        },
        error: (err) => {
